...
public:
    ...
    void exportToCSV() {
        ofstream outFile("Timetable(withoutRandom).csv");
        if (!outFile) {
        ...
        cout << "Timetable exported to Timetable(withoutRandom).csv" << endl;
        outFile.close();
    }
};
